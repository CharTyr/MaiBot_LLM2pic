...
class CustomPicAction:
    ...
    action_description = "生成并发送图片（纯文生图，二次元/动漫风格）。触发场景：用户让你画图/发图/自拍、要求看你长什么样/在干嘛、说\"画一张...\"\"给我看看...\"。当你判断需要用图而非纯文字回应时，应立即调用此工具。注意：如果用户发送了图片并要求修改/编辑，或明确要求写实风格，请使用 edit_picture 工具。"
...
class LLM2PicPlugin(MaiBotPlugin, _RuntimeBridgeMixin):
    ...
    @Action(
        ...
        parallel_action=False,
        action_prompt="""此动作让你能够编辑图片或生成写实风格图片。仅在用户有非常明确的编辑/修改意图时才触发。
