...
class CustomPicAction:
    ...
    action_description = "生成并发送图片（纯文生图，二次元/动漫风格）。触发场景：用户让你画图/发图/自拍、要求看你长什么样/在干嘛、说\"画一张...\"\"给我看看...\"。当你判断需要用图而非纯文字回应时，应立即调用此工具。注意：如果用户发送了图片并要求修改，或明确要求使用GPT/写实模型生成，请使用 edit_picture 工具。"

    # LLM判定提示词
    llm_judge_prompt = """
此动作让你能够从零生成并发送二次元/动漫风格的图片，用于回应群友想要"看到"某些视觉内容的请求。仅用于文生图。
...
- 用户明确提到使用 GPT/gpt/GPT模型/写实模型 生成图片
- 用户说"用GPT画..."、"GPT生成..."、"用gpt-image生成..."
- 用户要求生成写实/真实/照片级别的图片
...
class LLM2PicPlugin(MaiBotPlugin, _RuntimeBridgeMixin):
    ...
    @Action(
        ...
        action_require=[
            ...
            "当用户引用一张图片并描述修改需求时使用",
            "当用户明确提到使用GPT/gpt模型生成图片时使用",
            "当用户要求生成写实/真实风格的图片时使用",
        ],
...
【触发条件 - GPT 文生图】
1. 用户明确提到使用 GPT/gpt 模型生成图片（如"用GPT画..."、"GPT生成一张..."、"用gpt-image画..."）
2. 用户要求生成写实/真实/照片级别的图片（如"画一张写实的..."、"生成一张真实的..."）
3. 用户的需求明显更适合写实风格而非二次元（如真实人物照片、产品图、建筑效果图）
...
- 用户没有提到 GPT/写实/真实，且需求是普通的画图（应使用 draw_picture）
